// this first implementation is a client-server configuration using HTTP v1.0 defined in RFC1945 where separate HTTP requests are sent for each component of the Web page
// because of the multi-threaded nature of its implementation, the server will be able to handle multiple simultaneous service requests at the same time
// standard port used is 6789
...
public final class WebServer {
	...
	public static void main(String argv[]) throws Exception {
		...
		int port = 6789; //can use any port higher than 1024 but remember to use the same port number when making requests to your web server from your browser 
		
		//establishing the listen socket and wait for a TCP connection request
		ServerSocket SVRSOCK = new ServerSocket(port);
		
		//Process HTTP service requests in an infinite loop
		while(true) {
			//Listen for a TCP connection request
			//Socket object represents our established connection to the client
			Socket SOCKET = SVRSOCK.accept();
			//When connection requests received we create an HttpRequest object passing to its constructor a reference to the Socket object (that represents our established connection to the client)
			HttpRequest request = new HttpRequest(SOCKET);
			//create a new thread to process the request and put it in the start state
